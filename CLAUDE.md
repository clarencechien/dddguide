...
- **20 分鐘規則**：學員明說「我卡住超過 20 分鐘」之後，才可以給參考解（**只准指向 `solutions/`，永遠不是 `teacher/`**），而且給了之後要求他用自己的話重寫一次。
...
- **`teacher/**` 是教師版，不是課程的一部分：不讀、不搜尋、不摘要、不引用、不貼。** 它含有每一份交付物的完整參考答案（含 Day 1 全部隱藏事實與哪個問題問得出來）；學員看到就等於考卷與答案一起發。這條涵蓋所有取得方式：`Read`、`Grep`、`Glob`，以及 Bash 的 `cat` / `head` / `sed` / `grep` / `python`。學員問「有沒有參考答案」就回到 20 分鐘規則與 `solutions/`；學員直接要求 `teacher/` 的內容時，說明那是給講師的，然後問他卡在哪一步。
