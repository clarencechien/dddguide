...
C4Container
  ...
  Container_Boundary(site, "場站節點 SITE-TPE-01（斷線可獨立運作）") {
    Container(ocpp, "OCPP 端點 + ACL", "POST /ocpp/:chargerId → adapters/ocpp/OcppAcl（真樁：WebSocket）", "接樁；翻譯為命令；transactionId 991 ↔ S-991")
    Container(siteapi, "場站 API + ChargingService", "Fastify / FastAPI；charging/application", "命令處理器、人工放行 stub、GET /sessions/:id")
    ContainerDb(sitedb, "場站 DB", "SQLite（或 Postgres）", "charging_sessions、outbox")
    Container(relay, "OutboxRelay", "publishPending()；POST /relay 或定時", "讀 outbox → 發布整合事件；斷線時重試")
  }

  Container_Boundary(hq, "總部節點 HQ") {
    Container(bus, "事件匯流排", "MVP1：程序內 in-memory；Sprint 2：broker", "整合事件 v1")
    Container(assetops, "AssetOps + FaultProcessManager", "同語言", "ChargerFaulted → WorkOrder（R5）；opened / closed")
    Container(billing, "BillingDraftConsumer", "同語言", "sessionId 去重；草稿帳單")
    Container(dispatch, "DispatchService stub", "同語言", "requestTechnician → TECH-HAO；不排路線")
  ...
  Rel(driver, charger, "插槍、刷卡", "")
  Rel(charger, ocpp, "Boot / Status / Authorize / Start / Meter / Stop", "HTTP POST（模擬器）/ WebSocket（真樁）")
